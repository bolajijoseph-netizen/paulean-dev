      ...
      {Object.values(columns).map(col => (
        <div className="kcol" key={col.id}>
          <div className="kcol-header">
            ...
            <div className="kcol-count">{tasks.filter(t =>(t.currentPlan === col.id)||t.delegatedToPaulean&&col.id=='paulean').length}</div>
          </div>
          <div
            className="kcol-body"
          ...
          >
            {tasks.filter(t => (t.currentPlan === col.id)||t.delegatedToPaulean&&col.id=='paulean').map(t => (
              <TaskCard
                key={t.id}
                task={t}
            ...
            <div className="add-slot-btn" onClick={() => handlers.openAdd(col.id)}>＋ Add time slot</div>
          </div>
        </div>
      ))}
    </div>
